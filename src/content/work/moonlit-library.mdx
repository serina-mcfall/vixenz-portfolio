...
A quick note for anyone clicking through today: Render's free tier lets the server sleep after inactivity, so the first request may take around 30 seconds to wake it up. Everything after that responds normally. A move to Cloudflare Pages is on the roadmap; that will take the cold start out of the picture.
